...
int make_worker(std::string &new_file, const char* pipe_dir, std::map<pid_t,int> &worker_to_pipe, int &num_alive_workers, sigset_t block_set);
...
int gather_children(std::queue<pid_t> &available_workers, int &num_alive_workers, int listen_pid);
