...
int main(int argc, char* argv[]) {
    ...
    sigset_t block_set;
    while (sigfillset(&block_set) < 0) {
        if (errno != EINTR) {
            perror("manager: sigfillset");
            exit(EXIT_FAILURE);
        }
    }
    ...
    /* Main loop */
    std::queue<pid_t> available_workers;  // queue of available workers
    int num_alive_workers = 0;            // number of workers still alive
    std::map<pid_t,int> worker_to_pipe;   // map from worker's pid to the fd of the equivalent named pipe
            ...
            for (uint i = 0 ; i < nread ; i++) {
                ...
                /* When reaching a newline, we have read a complete file name, and we send it to a worker for processing */
                    ...
                    /* Else wake up an available worker */
                        ...
                        pid_t available_worker = available_workers.front();
    ...
    for(std::map<pid_t, int>::const_iterator it = worker_to_pipe.begin() ; it != worker_to_pipe.end() ; ++it) {
