...
int main(int argc, char* argv[]) {
    ...
    while ((pipe_fd = open(argv[1], O_RDONLY | O_NONBLOCK)) < 0) {
        if (errno != EINTR) {
    ...
    /* Get flags from pipe */
    int opts;
    while ((opts = fcntl(pipe_fd, F_GETFL, 0)) < 0) {
        if (errno != EINTR) {
            perror("worker: fcntl");
            exit(EXIT_FAILURE);
        }
    ...
    while (fcntl(pipe_fd, F_SETFL, opts & ~O_NONBLOCK) < 0) {
        if (errno != EINTR) {
            perror("worker: fcntl");
            exit(EXIT_FAILURE);
        }
    ...
    while (sigfillset(&block_set) < 0) {
        if (errno != EINTR) {
            perror("worker: sigfillset");
            exit(EXIT_FAILURE);
        }
    }

    /* If everything successful, stop and wait for the manager to wake us up */
    if (raise(SIGSTOP) != 0) {
